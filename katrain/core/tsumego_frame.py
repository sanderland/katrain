...
def tsumego_frame_stones(stones, komi, black_to_play_p, ko_p):
    ...
    isize, jsize = sizes
    ijs = flatten([[{"i": i, "j": j, "black": h.get("black")} for j, h in enumerate(row) if h.get("stone")] for i, row in enumerate(stones)])
    if len(ijs) == 0:
        return []
    ...
    flip_spec = [False, False, True] if imin < jmin else [need_flip_p(imin, imax, isize), need_flip_p(jmin, jmax, jsize), False]
...
def guess_black_to_attack(extrema, sizes):
    return sum([height2(z, sizes) for z in extrema])
