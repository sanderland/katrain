...
from katrain.core.constants import HOMEPAGE, VERSION, SGF_COMMENTS_MARKER, SGF_SEPARATOR_MARKER, SGF_NOTE_MARKER, PROGRAM_NAME
...
class GameNode(SGFNode):
    ...
    def add_list_property(self, property: str, values: List):
        if property == "KT":
            try:
                ...
                self.analysis = bson.loads(gzip.decompress(base64.standard_b64decode(values[0])))
                self.analysis_loaded = True
            except Exception as e:
                print(e)
        elif property == "C":
            comments = [  # strip out all previously auto generated comments
                c for v in values for c in v.split(SGF_SEPARATOR_MARKER)
            ]
            self.note = "".join(c for c in comments if SGF_NOTE_MARKER in c)
            super().add_list_property(
                property,
                [c for c in comments if c.strip() and SGF_COMMENTS_MARKER not in c and SGF_NOTE_MARKER not in c],
            )
        else:
    ...
    def sgf_properties(
        ...
        print(save_analysis, self.analysis_complete)
        if save_analysis and self.analysis_complete:
            try:
                ...
                properties["KT"] = [base64.standard_b64encode(gzip.compress(bson.dumps(analysis))).decode("utf-8")]
            except Exception as e:
                print(e)
        ...
        if self.is_root:
            comments = [
                i18n._("SGF start message") + SGF_COMMENTS_MARKER,
                *comments,
                f"SGF with generated by {PROGRAM_NAME} {VERSION}{SGF_COMMENTS_MARKER}.",
            ]
        if note:
            comments.append(f"\nNote: {self.note}{SGF_NOTE_MARKER}")
        if comments:
