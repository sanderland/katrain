...
from katrain.gui.style import WHITE, STONE_COLORS, LIGHTGREY, BACKGROUND_COLOR, OUTLINE_COLORS, LIGHTER_BACKGROUND_COLOR, YELLOW, STONE_TEXT_COLORS
...
class MoveTreeCanvas(Widget):
    ...
    def on_touch_up(self, touch):
        if touch.button=='left':
            node, (x,y) = min(self.move_xy_pos.items(),key=lambda n_xy: abs(n_xy[1][0]-touch.x)+abs(n_xy[1][1]-touch.y) )
    ...
    def draw_move_tree(self, current_node):
        ...
        with self.canvas:
            Color(*YELLOW)
            Rectangle(pos=[c-self.move_size/2-spacing/2 for c in self.move_xy_pos[current_node]],size=(self.move_size+spacing,self.move_size+spacing))
            Color(*LIGHTGREY)
...
class MoveTree(ScrollView,BackgroundMixin):
    ...
    def __init__(self, **kwargs):
        ...
        self.redraw_tree_trigger = Clock.create_trigger(lambda _dt: self.move_tree_canvas.draw_move_tree(self.current_node))
