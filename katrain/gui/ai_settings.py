...
class ConfigAIPopupContents(QuickConfigGui):
    ...
    def build(self, _):
        ...
        bottom_bl.add_widget(SizedMDFlatButton(text=f"Apply", on_press=lambda _: self.update_config(False)))  # raised?
        bottom_bl.add_widget(self.info_label)
        bottom_bl.add_widget(SizedMDFlatButton(text=f"Apply and Save", on_press=lambda _: self.update_config(True)))
